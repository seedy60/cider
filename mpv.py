...
if hasattr(backend, "mpv_get_sub_api"):
    _handle_func(
        "mpv_get_sub_api", [MpvSubApi], c_void_p, notnull_errcheck, deprecated=True
    )

    _handle_gl_func(
        "mpv_opengl_cb_set_update_callback",
        [OpenGlCbUpdateFn, c_void_p],
        deprecated=True,
    )
    _handle_gl_func(
        "mpv_opengl_cb_init_gl",
        [c_char_p, OpenGlCbGetProcAddrFn, c_void_p],
        c_int,
        deprecated=True,
    )
...
class MPV(object):
    ...
    def prepare_and_wait_for_property(
        self, name, cond=lambda val: val, level_sensitive=True
    ):
    ...
    def _handle_key_binding_message(
        self, binding_name, key_state, key_name=None, key_char=None
    ):
    ...
    def register_stream_protocol(self, proto, open_fn=None):
        ...
        def decorator(open_fn):
            ...
            def open_backend(_userdata, uri, cb_info):
                ...
                close = cb_info.contents.close = StreamCloseFn(
                    lambda _userdata: frontend.close()
                )

                seek, size, cancel = None, None, None
                if hasattr(frontend, "seek"):
                    seek = cb_info.contents.seek = StreamSeekFn(
                        lambda _userdata, offx: frontend.seek(offx)
                    )
                if hasattr(frontend, "size") and frontend.size is not None:
                    size = cb_info.contents.size = StreamSizeFn(
                        lambda _userdata: frontend.size
                    )
