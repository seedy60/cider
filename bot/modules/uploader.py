...
class Uploader:
    ...
    def __call__(self, track: Track, user: User, ttclient) -> None:
    ...
    def run(self, track: Track, user: User, ttclient) -> None:
        error_exit = False
        if track.type == TrackType.Default:
            ...
            file_path = track.url
        command_id = ttclient.send_file(ttclient.channel.id, file_path)
        file_name = os.path.basename(file_path)
        while True:
            try:
                file = ttclient.uploaded_files_queue.get_nowait()
                if file.name == file_name:
                    break
                else:
                    ttclient.uploaded_files_queue.put(file)
            except Empty:
                pass
            try:
                error = ttclient.errors_queue.get_nowait()
                if (
                    ...
                    ttclient.errors_queue.put(error)
            except Empty:
                pass
        ...
        if self.config.general.delete_uploaded_files_after > 0:
            ...
            return
        time.sleep(timeout)
        ttclient.delete_file(file.channel.id, file.id)
